...
\noindent Make a subclass \pythoninline{Midpoint} in the \pythoninline{ODESolver} hierarchy from Section E.3 for solving ordinary differential equations with Euler's midpoint method. This method computes
\begin{align*}
...
\begin{Problem}{\textbf{Implement Heun's method; function}}
\paragraph{a)}
\noindent
Implement Heun's method specified in formula (E.36-E.37) on page 779 in the book. Use a plain function \pythoninline{heuns_method} of the type shown in Sect. E.1.3. Construct a test problem where you know the analytical solution. 
Implement a test function \pythoninline{test_heuns_against_hand_calculations()} where you compare your own hand calculated results for $u_0$, $u_1$ and $u_2$ with those in the program.
...
\begin{Problem}{\textbf{Solve an ODE until constant solution}}\\
\noindent
...
\paragraph{a)} 
Make a \pythoninline{class Problem} containing the parameters $h$ and $T_s$ as data attributes. Let these parameters be set in the constructor. Implement the right-hand side of the ODE in a \pythoninline{__call__(self, T, t)} method. We will use a class from the \pythoninline{ODESolver} hierarchy to solve the ODE, so you shall also include the \pythoninline{terminate} function as a method in class \pythoninline{Problem} (reading the first part of Sect. E.3.6 may be useful). \newline
Create a stand-alone function \pythoninline{estimate_h(t1, Ts, T0, T1)} to estimate the $h$ parameter based on the initial temperature and one data point $(t_1, T(t_1))$. You can use this function to estimate a value for $h$ prior to calling the constructor in the \pythoninline{Problem} class.

\paragraph{b)}
Implement a test function \pythoninline{test_Problem()} for testing that class \pythoninline{Problem} works. It is up to you to define how to test the class.
...
For each $T_s = [20, 25]$, solve the ODE numerically by a method of your choice from the \pythoninline{ODESolver} hierarchy. Remember to send both the array of t-values and \pythoninline{<instance>.terminate} as parameters when calling the \pythoninline{solve} method (because \pythoninline{terminate=None} by default). Plot your two $T$-arrays of different solutions in the same plot. If the \pythoninline{terminate} method works all right, your graphs should be cut when close enough to the asymptotic values $T_s$ of room temperature.
...
\paragraph{a)}Make two subclasses \pythoninline{Heun} and \pythoninline{RungeKutta2} in the \pythoninline{ODESolver} hierarchy from Section E.3 in the book for solving ordinary differential equations with Heun's method and the 2nd-order Runge-Kutta method.  
