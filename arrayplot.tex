...
The topics of this chapter are array-computing and plotting. The exercises correspond to
...
Write a function \py{abs_approx(x,N)} that calculates the sum and
...
\paragraph{a)}
Make a function \pythoninline{plot_line(p1,p2)} that takes two points as arguments
and plots the line between them. The two arguments should be lists or tuples specifying
$x$- and $y$-coordinates, i.e., \pythoninline{p1 = (x1,y1)}. Demonstrate that the
...
function \py{complete_graph(points)} that takes a list of points and plots the
...
Hint: Modify the \pythoninline{plot_line} function from question a) so that it only calls
